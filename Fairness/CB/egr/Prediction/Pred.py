...
base_path = "./Fairness/CB/egr/Prediction"
...
model_ITC = joblib.load("./Fairness/CB/egr/InviteToConduct/model/model.pkl")
...
def conduct_predictions(model, pred_data, features):
    X_pred = pred_data[features]

    predictions = egr_predict_proba(model, X_pred)
...
model_OTR = joblib.load(f"Fairness/CB/egr/OfferToRegister/model/model.pkl")
...
def enrollment_prediction(model, pred_data, pred_name, base_path):
    ...
    pred_data["predicted_enrollment"] = (pred_data["enrollment_likelihood"] > 0.35) & (
        pred_data["interview_likelihood"] > 0.45
    )
