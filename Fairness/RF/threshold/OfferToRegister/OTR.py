...
base_path = "./Fairness/RF/threshold/OfferToRegister/"
...
def classification_model(
    ...
    sensitive_values = X_val["ethnicity"]
    ...
    fair_preds = threshold_optimizer.predict(
        X_val,
        sensitive_features=sensitive_values
    )
    ...
    # Evaluation
    print("\n--- Fairness-Aware Predictions Evaluation ---")
    print("Accuracy:", accuracy_score(y_val, fair_preds))
    print("Balanced Accuracy:", balanced_accuracy_score(y_val, fair_preds))
    print("F1 Score:", f1_score(y_val, fair_preds))
    print(classification_report(y_val, fair_preds))
    ...
    return calibrated_clf, threshold_optimizer


def likelihood_prediction(
    optimizer, pred_data, pred_name, features, base_path
):
    ...
    #     pred_data["registered"], predictions, save_path=save_dir
    # )
...
calibrated_clf, optimizer = classification_model(
    training_data,
    feature_columns,
    target_column,
    param_grid,
...
save_model(calibrated_clf, base_path)
joblib.dump(optimizer, f"{base_path}/model/optimizer.pkl")
...
for pred_name, pred_data in prediction_datasets.items():
    likelihood_prediction(
        optimizer, pred_data, pred_name, feature_columns, base_path
    )
