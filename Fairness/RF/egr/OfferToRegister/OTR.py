...
base_path = "./Fairness/RF/egr/OfferToRegister/"
...
def classification_model(
    ...
    numerical_features = ["age", "salary_range", "character_count", "payment_amount"]
    ...
    base_model = RandomForestClassifier(
        n_estimators=param_grid['classifier__n_estimators'][0],
        max_depth=param_grid['classifier__max_depth'][0],
        min_samples_split=param_grid['classifier__min_samples_split'][0],
        min_samples_leaf=param_grid['classifier__min_samples_leaf'][0],
    ...
    likelihood_df.to_excel(f"{save_dir}/enrollment_likelihood_val.xlsx", index=False)
...
def likelihood_prediction(
    calibrated_clf, pred_data, pred_name, features, best_threshold, base_path, preprocessor_pipeline
):
    X_raw = pred_data[features]
    X_proc = preprocessor_pipeline.transform(X_raw)

    if hasattr(X_proc, "toarray"):
        X_proc = X_proc.toarray()

    if hasattr(calibrated_clf, "predictors_"):
        likelihood_calibrated = egr_predict_proba(calibrated_clf, X_proc)
    else:
        likelihood_calibrated = calibrated_clf.predict_proba(X_proc)[:, 1]
...
param_grid = {
    'classifier__n_estimators': [500],  # Default: 100
    'classifier__max_depth': [20],  # Default: None (expand fully unless limited by other params)
    'classifier__min_samples_split': [5],  # Default: 2
    'classifier__min_samples_leaf': [1],  # Default: 1
    'classifier__class_weight': ['balanced'],  # Default: None
}
