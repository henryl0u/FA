...
base_path = "./Fairness/LR/egr/Prediction"
...
model_ITC = joblib.load("./Fairness/LR/egr/InviteToConduct/model/model.pkl")
# Load preprocessor used for InviteToConduct model
preprocessor_ITC = joblib.load("./Fairness/LR/egr/InviteToConduct/model/preprocessor.pkl")
...
def conduct_predictions(model, pred_data, features, preprocessor=None):
    X_pred_raw = pred_data[features]

    if hasattr(model, "predictors_"):
        # Use fitted preprocessor if this is an EGR model
        if preprocessor is None:
            raise ValueError("Preprocessor required for EGR model predictions.")
        X_pred_proc = preprocessor.transform(X_pred_raw)
        predictions = egr_predict_proba(model, X_pred_proc)
    else:
        predictions = model.predict_proba(X_pred_raw)[:, 1]
...
def plot_confusion_matrix(y_true, y_pred, clf, save_path=None):
    if hasattr(clf, "predictors_"):
        base_clf = clf.predictors_[0]
    else:
        base_clf = clf
...
model_OTR = joblib.load("./Fairness/LR/egr/OfferToRegister/model/model.pkl")
# Load preprocessor used for InviteToConduct model
preprocessor_OTR = joblib.load("./Fairness/LR/egr/OfferToRegister/model/preprocessor.pkl")
...
def likelihood_prediction(calibrated_clf, pred_data, features, preprocessor=None):
    X_test_raw = pred_data[features]

    if hasattr(calibrated_clf, "predictors_"):
        if preprocessor is None:
            raise ValueError("Preprocessor required for EGR model predictions.")
        X_test = preprocessor.transform(X_test_raw)
        likelihood_calibrated = egr_predict_proba(calibrated_clf, X_test)
    else:
        X_test = X_test_raw if preprocessor is None else preprocessor.transform(X_test_raw)
...
pred60_data = likelihood_prediction(
    model_OTR,
    pred60_data,
    features_OTR,
    preprocessor=preprocessor_OTR
)
