...
base_path = "./Fairness/CB/CourseCompletion/Base"
...
# Define a dictionary for the mapping of course completion statuses
# After interview is defined as no completion since we look at the course completion data before the interview
replacement_map = {
    "After Interview": False,
    "Before Interview": True,
    "No Completion": False,
}


# Function to apply replacements and ensure consistent types
def replace_course_completion_status(df):
    # Step 1: Map the values using the defined mapping
    df["course_completion_status"] = df["course_completion_status"].map(replacement_map)

    # Step 2: Use `fillna` but ensure that the column is treated as boolean beforehand
    df["course_completion_status"] = (
        df["course_completion_status"].astype(pd.BooleanDtype()).fillna(False)
    )

    # Step 3: Cast the result to boolean type (non-nullable)
    df["course_completion_status"] = df["course_completion_status"].astype(bool)

    return df


# Apply the replacements to each relevant DataFrame
training_data = replace_course_completion_status(training_data)
pred66_data = replace_course_completion_status(pred66_data)
pred67_data = replace_course_completion_status(pred67_data)
...
def plot_likelihood_distribution(y_true, y_probs, save_path=None):
    ...
    plt.title("Distribution of Predicted Likelihoods by Actual Course Completion")
    plt.xlabel("Predicted Likelihood to Complete")
    plt.ylabel("Count")

    # Set custom legend labels
    plt.legend(title="Actual Completion", labels=["Completed", "Not Completed"])
    if save_path:
    ...
    plt.title("Box Plot of Predicted Likelihoods by Actual Course Completion")
...
def likelihood_prediction(calibrated_clf, pred_data, pred_name, features, base_path):
    ...
    likelihood_df["CC_likelihood"] = likelihood_calibrated

    save_dir = f"{base_path}/prediction/{pred_name}"
    os.makedirs(save_dir, exist_ok=True)

    likelihood_df.to_excel(f"{save_dir}/CC_likelihood_{pred_name}.xlsx", index=False)
    plot_likelihood_distribution(
        pred_data["course_completion_status"], likelihood_calibrated, save_path=save_dir
    )
...
target_column = "course_completion_status"
